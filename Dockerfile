...
RUN apt-get -y update && apt-get -y dist-upgrade && apt-get -y install binutils jsvc mongodb ufw default-jre-headless && dpkg -i /tmp/unifi_sysvinit_all.deb && rm -f /tmp/unifi_sysvinit_all.deb && useradd --system --user-group --create-home unifi
